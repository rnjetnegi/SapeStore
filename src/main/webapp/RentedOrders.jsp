			...
			<div id="header">
				...
				<nav>
					<ul class="nav">
						...
						<li class="active"><a href="/SapeStore/manageOrders">Manage
								Orders</a></li>
		...
		<section style="height: 650px; margin-bottom: 20px;">
			<c:forEach items="${toDispatchOrdersList}" var="entry">
				...
				<c:forEach items="${entry.value}" var="current">
   				 ${current.customerId}      ${current.orderNumber}       ${current.itemName}         ${current.type}
   				 ${current.purchasePrice} 			${current.rentAmount} 			${current.expectedReturnDate }<br>
				</c:forEach>
				<br>
			</c:forEach>
